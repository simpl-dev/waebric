...
object WaebricSimplConfig {
  ...
  val instance = new WaebricSimplConfig()
  val languageId = "waebric_simpl"
  val language = LanguageRegistry.findLanguage(languageId)
  val pluginId = "waebric_simpl"
...
class WaebricSimplConfig extends APluginConfig {
  /** Parses the input using the correct grammar. */
  def parse(ctx: ParseCtx) {
      ctx parse new WaebricSimplGrammar()
  }
